...
import useCOLADBTableData from '../hooks/useCOLADBTableData'
import { useMutationDeleteCOLADB } from '../query/useMutationCOLADB'
import COLADBDetailTable from './COLADBDetailTable'

type COLADBProps = {
  COLADBData: COLADBResponseDataType[]
  handleNewLaunchClick: () => void
}

const COLADBTable = ({ COLADBData, handleNewLaunchClick }: COLADBProps) => {
  const [isDetailClicked, setIsDetailClicked] = useState<boolean>(false)
  const [selectedCOLADBId, setSelectedCOLADBId] = useState<string>('')
  const { columns, data } = useCOLADBTableData(COLADBData)
  const tableContainerRef = useRef<HTMLDivElement>(null)
  const tableRef = useRef<HTMLTableElement>(null)
  const { isVisible: isCollisionAvoidanceClicked } = useModal('COLLISIONAVOIDANCE')
  const { mutate } = useMutationDeleteCOLADB()

  const handleDetailClick = async (id: string) => {
    setSelectedCOLADBId(id)
  ...
  const { getTableProps, getTableBodyProps, headerGroups, prepareRow, rows } = useTable(
    ...
    (hooks) => {
      hooks.visibleColumns.push((columns: Column<COLADBResponseDataType>[]) => {
        columns[3] = {
          Header: 'Status',
          Cell: ({ row, value }: CellProps<COLADBResponseDataType>) => (
            <>
  ...
  useEffect(() => {
    ...
    tableContainerRef.current.style.visibility = isCollisionAvoidanceClicked ? 'visible' : 'hidden'
    tableContainerRef.current.style.transform = `translateX(${
      isCollisionAvoidanceClicked ? '0' : '40rem'
    })`
  })

  return (
    <>
      <COLADBTableWrapper isVisible={isCollisionAvoidanceClicked} ref={tableContainerRef}>
        {!isDetailClicked ? (
          <>
            <div className="launch-conjunction-header">
              <h1 className="title">Collision Avoidance</h1>
          ...
          <COLADBDetailTable handleBackButton={handleBackButton} COLADBId={selectedCOLADBId} />
        )}
      </COLADBTableWrapper>
    </>
  )
...
export default COLADBTable

type COLADBWrapperProps = {
  isVisible: boolean
}

const COLADBTableWrapper = styled.div<COLADBWrapperProps>`
  width: 500px;
  padding: 1.5rem;
