...
import { useMutationPostCOLADB } from '../query/useMutationCOLADB'
import WarningModal from '@app.components/modal/WarningModal'
import { PrimaryButton } from '@app.components/button/Button'
import { useQueryGetCOLADBSampleDownload } from '../query/useQueryCOLADB'
import { DataResponseType } from '@app.modules/types'

type AssessmentModalProps = {
  handleAssessmentModalClose: () => void
  setIsCOLADBTableOpen: React.Dispatch<React.SetStateAction<boolean>>
  setIsSuccessModalOpen: React.Dispatch<React.SetStateAction<boolean>>
  refetchCOLADBData: <TPageData>(
    options?: RefetchOptions & RefetchQueryFilters<TPageData>
  ) => Promise<QueryObserverResult<DataResponseType<COLADBResponseDataType[]>, unknown>>
}
...
const AssessmentModal = ({
  ...
  setIsCOLADBTableOpen,
}: AssessmentModalProps) => {
  const { isVisible, handleCloseModal, handleSetModal } = useModal('COLLISIONAVOIDANCE')
  const [isSubmitDisabled, setIsSubmitDisabled] = useState<boolean>(true)
  const [thresholdValue, setThresholdValue] = useState<number | null>(null)
  const [inputFile, setInputFile] = useState<File>()
  const [fileName, setFileName] = useState<string>('')
  const { mutateAsync } = useMutationPostCOLADB()
  const [isErrorModalVisible, setIsErrorModalVisible] = useState(false)
  const [errorMessage, setErrorMessage] = useState<string>('')
  const { refetch } = useQueryGetCOLADBSampleDownload()
  ...
  const handleSubmit = async () => {
    await mutateAsync({
      ...
      .then(() => {
        ...
        setIsCOLADBTableOpen(true) // FIXME: Is it necessary?
        handleAssessmentModalClose()
        handleSetModal()
      })
      .catch((error) => {
